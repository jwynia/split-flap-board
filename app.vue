<template>
  <div id="app">
    <SplitFlapContainer mode="slow" :value="message" :maxLength="maxLength" />
  </div>
  <div>{{ maxLength }}</div>
</template>
...
export default {
  ...
  created() {
    // look at the queryString for messageCollectionName
    const urlParams = new URLSearchParams(window.location.search);
    const messageCollectionName = urlParams.get("messageCollectionName");
    //if there's no messageCollectionName, use the default
    if (!messageCollectionName) {
      console.log("No messageCollectionName specified, using default messages");
      this.possibleMessages = this.possibleMessages;
      this.changeText();
    } else {
      //Load the messages from a file named messageCollectionName.json in assets
      console.log(`Loading messages from ${messageCollectionName}.json`);
      fetch(`/data/${messageCollectionName}.json`)
        .then(response => response.json())
        .then(data => {
          const longestEntryLength = data.reduce((maxLen, str) => Math.max(maxLen, str.length), 0);
          this.maxLength = longestEntryLength;
          this.possibleMessages = data;
          this.changeText();
        });
    }
    setInterval(() => {
